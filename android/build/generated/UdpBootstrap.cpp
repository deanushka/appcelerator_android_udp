...
static Handle<Value> Udp_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Udp.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::UdpBindings::lookupGeneratedInit(
...
static void Udp_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Udp_getBinding)->GetFunction());
}

static void Udp_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::UdpBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry UdpBinding = {
	"ti.udp",
	Udp_init,
	Udp_dispose
};
...
Java_ti_udp_UdpBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.udp", &UdpBinding);
	titanium::KrollBindings::addExternalLookup(&(::UdpBindings::lookupGeneratedInit));
}
